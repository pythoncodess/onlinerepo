...
client = commands.Bot(description="Here is some command for you", command_prefix=commands.when_mentioned_or("mp!"), pm_help = False)


async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='Master Joker | mp!help'))
...
522431572645904405 if message.server is None and message.author != client.user:
...
async def dm(ctx, user: discord.Member, *, msg: str):
    try:
        ...
        await client.say("Error :x:. Make sure your message is shaped in this way: mp!dm [tag person] [msg]")
...
async def info(ctx):
    ...
    await client.say('***THIS BOT IS MADE BY MASTER JOKER AND IF YOU NEED TO INVITE THIS BOT SO WRITE `mp!botinvite` THANKS***')

@client.command(pass_context = True)

async def botinvite(ctx):

    await client.say('https://discordapp.com/api/oauth2/authorize?client_id=522422075399602210&permissions=8&scope=bot')
...
async def kick(ctx,user:discord.Member):
    if user.server_permissions.      await client.say('**He is mod/admin and i am unable to kick him/her**')
      return
    else:
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- Master joker", color=0x00ff00)
